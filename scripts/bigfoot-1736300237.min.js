/**
 * This is a manually updated version of bigfoot to fix what I percieve to be an issue with
 * numerical footnotes. When a footnote is reused (e.g. "reference[^footnote] and
 * referencing that same thing again[^footnote]"), bigfoot renders the result as:
 *
 ...
 *   1. The footnote contents
 *
 * Any modern-day replacements like littlefoot (which I intend to get to, sometime) must
 * also include this fix.
 *
 * v2.1.4
 */
(function() {
  (function($) {
    return $.bigfoot = function(options) {
      ...
      footnoteInit = function() {
        ...
        _results = [];
        relevantFNLinkToNumber = new Map();
        for (i = _i = 0, _ref = footnotes.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          ...
          footnoteNumToPaint = 0;
          if (settings.numberResetSelector != null) {
          	if (!relevantFNLinkToNumber.has($relevantFNLink.attr("data-footnote-ref"))) {
          		...
          		relevantFNLinkToNumber.set($relevantFNLink.attr("data-footnote-ref"), footnoteNum);
          		footnoteNumToPaint = footnoteNum;
          	} else {
          		footnoteNumToPaint = relevantFNLinkToNumber.get($relevantFNLink.attr("data-footnote-ref"));
          	}
          ...
          footnoteButton = settings.buttonMarkup.replace(/\{\{FOOTNOTENUM\}\}/g, footnoteNumToPaint).replace(/\{\{FOOTNOTEID\}\}/g, footnoteIDNum).replace(/\{\{FOOTNOTECONTENT\}\}/g, footnoteContent);
